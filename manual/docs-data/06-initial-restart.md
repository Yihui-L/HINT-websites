...
2.0.0 直接演化 B，不演化 A。设 B₀ 为完整线圈集合的平滑核心场，V 为外部 B virtual-casing 积分：
...
日志在64个表面探针上报告外侧B₀+V与wout场的法向差、矢量差、wout自身Bn及积分误差估计；另报内域B₀+B₁=Bwout的代数保真度和物理内行的差分散度。小积分误差不保证边界两侧一致。

后续kdivb扩散响应场散度，不清理固定B₀。保留LCFS跳跃不等于承诺它一定在有限次迭代后消失。相容B插值保留面磁通散度，不暗中修正数据。

## follow

paths.output_file同时是输入与追加目标。只接受同版本schema 20，恢复B₀、当前B/p/v/s、初始冻结B₁参考、轴热启动、攀升状态和每记录求解设置。未指定的数值控制继承；不能改变start_point。无需外部wout、线圈或壁文件。
