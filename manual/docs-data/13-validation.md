...
1.1.1 本轮并行审查完成完整本地 CPU 回归：debug 300 项、wall 297 项通过，各有 10 项 subtest；两版均在远程执行真实两进程 MPICH/JAX 测试，每 rank 8 项通过，并执行 RTX 5090 上线圈积分、矢势推进与第一壁约束的制造解检查。实际 NCSX 线圈数据的 576 点积分吞吐量对照见[加速说明](#acceleration)。这不是完整生产算例的初始化精度或收敛性保证，也未进行多节点性能扫描。不作“已证明不存在 bug”或“所有平衡均收敛”的声明。
