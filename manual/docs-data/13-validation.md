...
本轮核对两版源码、参数、分支、存储字段和示例，运行完整 CPU 回归、GPU/JAX 矢势推进制造解和真实两进程 MPI 测试；网站参数由源码重新生成。**未重新运行 NCSX 生产算例、VMEC 响应场初始化 benchmark、扫参或跨节点性能测试。** 不作“已证明不存在 bug”或“所有平衡均收敛”的声明。
...
- 新线圈背景场通过配套离散旋度和连续旋度保无散，不再限幅。旧 B-only checkpoint 仅保留分析读取，不可直接用当前主程序 follow。
