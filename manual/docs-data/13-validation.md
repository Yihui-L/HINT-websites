...
1.3.1 检查包括两套CPU完整回归、RTX 5090/JAX float64直接A/路径专项、真实双进程MPICH/JAX回归和静态检查。新增越过2048节点的自动细化、少量困难目标的批大小控制、非有限值保护、空MPI目标进程和不等长细化测试。此前直接A的内外/表面极限、规范和环面孔洞测试继续保留。未测试多物理节点或OpenMPI专属实现。详见 [debug验证报告](source:debug:docs/VERIFICATION.md) 与 [wall验证报告](source:wall:docs/VERIFICATION.md)。
