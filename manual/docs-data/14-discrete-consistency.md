...
当前两版推进 (A₁,v)，而非 B₁。固定 A₀来自有限圆截面线圈源积分；B₀=curl_h A₀、B₁=curl_h A₁、B=B₀+B₁。J₁=curl_h B₁（归一化）进入完整洛伦兹力。时间规范下：
...
VMEC LCFS 全环面 virtual-casing 积分、近源正则化和初始加权离散修正保留。随后一次经验证的旧 C4 连续拟合供给节点 A₁，新 curl_h 得到用于演化和冻结边界的初始 B₁。转换必须报告平均矢量差/平均场强和最大矢量差/平均场强；分别超过 1% 或 5% 时拒绝，不通过调电流掩盖误差。此步骤不是每外步重复的成本。
