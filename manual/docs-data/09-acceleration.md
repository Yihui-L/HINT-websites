...
| 初始 B→A | 同一离散 curl 的 FFT/SVD 直接求解，MPI 分配 Fourier 模式，CPU/GPU 变换 | 根进程两个小 SVD、全局归约 |
...
旧 NCSX 初始化失败发生在连续矢势拟合后重新采样为节点 A 的转换阶段，未进入 MHD 演化。新路径直接使用演化配套的四阶离散 curl 求解节点 A，以分离 FFT/SVD 取代未充分收敛的迭代拟合和二次表示转换。原 1% 平均 / 5% 峰值场保真度阈值保留。此转换只在 VMEC 初始化运行一次，follow 和 Step-A 不运行逆求解。

该设计遵循 [JAX 异步调度文档](https://docs.jax.dev/en/latest/async_dispatch.html)：主机读取设备结果会同步，频繁的小批回传会破坏计算重叠。单块 GPU 的算力和带宽仍有限，不应使用更多进程竞争同一设备。第一壁约束矩阵批量组装、MPI 根进程构建后广播，约束本身未变。后处理取样最多两批在途，避免无限累积设备结果。
