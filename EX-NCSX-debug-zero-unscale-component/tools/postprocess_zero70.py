"""Read-only, complete step-70 figure export using installed HINT-debug APIs."""
import argparse
import hashlib
import json
import os
...
def save(result, name):
    ...
    if name.startswith('convergence_'):
        for axis in axes:
            axis.axvline(21, color='0.6', ls=':', lw=1)
    ...
    result.figure.suptitle('NCSX | HINT-debug 2.3.0 | outer 70 | zero / unscaled\n' + old_title,
                          fontsize=12)
...
def frame(backend='gpu', engine='jax'):
    ...
    p = HintPlots(RUN/'hint_debug_ncsx_zero.nc', outer_step=70,
    ...
    assert p.state.outer_step == 70 and config['scale_after'] is False
...
def static(p):
    ...
    for index in range(0, len(ordinary), 4):
        save(p.time_series(ordinary[index:index+4], ramp_end=20),
    ...
    for method in ('ad', 'fd4'):
        for field in ('vacuum','response','total'):
            ...
            if keys:
                save(p.time_series(keys, ramp_end=20), f'convergence_{method}_{field}')
...
def summary(p):
    ...
    events = [json.loads(line) for source in (RUN/'progress.jsonl', RUN/'follow-to070/progress.jsonl') for line in source.read_text().splitlines()]
    rows = [dict(r, outer_step=r['diagnostic']['outer_step']) for r in events
            if r['event']=='outer_complete' and r['diagnostic']['outer_step']<=70]
    assert len(rows) == 70 and any(r['event']=='finished' for r in events)
    ...
    for axis, key in zip(axes, ['Step-A','Step-B','outer_elapsed_s'], strict=True):
        ...
        axis.axvline(20, color='0.3', ls='--', lw=1, label='Ramp complete')
        axis.axvline(21, color='0.6', ls=':', lw=1, label='Follow checkpoint')
        axis.legend(fontsize=8)
        axis.grid(alpha=.25)
    fig.suptitle('NCSX | zero start | scale_after=false | 70 completed outer iterations')
    fig.savefig(FIG/'convergence_wall_clock_timings.png',dpi=300)
    plt.close(fig)
    (OUT/'run_summary.json').write_text(json.dumps(dict(
        version=hint_debug.__version__, outer_step=70, record=int(p.record),
        source=str(p.path), initialization=[r for r in events if r['event']=='initialization_complete'],
        last=dict(rows[-1], magnetic_statistics=[e for e in events if e['event']=='magnetic_diagnostics' and e['outer_step']==70][-1]), finished=[r for r in events if r['event']=='finished'],
        wall_clock_steps=rows, fields=list(FIELDS), metrics=available,
        initial_failure=[e for e in events if e['event']=='failed'],
        follow_submission=[e for e in events if e['event']=='submitted' and e.get('checkpoint_step')==21],
    ),indent=2)+'\n')
...
def histories(p):
    ...
    if rows:
        assert [r['outer'] for r in rows]==list(range(71))
    ...
    for title,quantities,mode in [
        ...
        for axis,name in zip(axes[:,0],quantities,strict=True):
            ...
            axis.legend(fontsize=8)
        fig.suptitle(f'NCSX | outer 0-70 | scale_after=false | volume weights R | {title}')
...
def iota(p):
    ...
    fig.suptitle('HINT-debug outer 70 | dense initial-VMEC-s scan | component interpolation')
    fig.savefig(FIG/'profile_rotational_transform_initial_vmec_s.png',dpi=300)
    plt.close(fig)


def existing(p):
    final=RUN/'final-poincare-step070-wall-coverage'
...
if __name__=='__main__':
    ...
    p=frame()
    log('loaded',version=hint_debug.__version__,outer=70,record=int(p.record))
