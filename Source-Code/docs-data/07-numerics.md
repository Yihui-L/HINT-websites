...
初始化分别保存源表达式 JAX AD 与插值器 JAX AD，二者不能混用；每个外迭代的真空、响应和总场保存插值器 AD 和网格 FD4 统计，定义见“输出信息”和“收敛性与平均方式”。FD4 使用守恒柱坐标中心模板及既有响应场边界闭合，仅作离散残差诊断，不把粗网格差分当成连续场精确导数。Step-B 既有 divb 字段仍为插值器 AD，不改变含义。`solver.step_b.kdivb=1e-4`、旋度、边界闭合及可选演化投影保持原算子。RBF 的小散度不能证明场值、磁面或力平衡正确。线圈独立场值和节点保真检验仍保留。
