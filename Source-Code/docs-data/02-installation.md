...
5. 阅读启动诊断：几何残差、平滑核心线圈场散度、原始 VMEC 响应场/LCFS 跳跃和边界误差。初始化不投影修正 LCFS 磁场。
6. 按完整 checkpoint 评估力、速度、散度和轴闭合，不仅看日志是否继续输出。

文档内提供[debug 输入模板](docs-data/templates/debug/main.toml)和[wall 输入模板](docs-data/templates/wall/main.toml)的源码原样副本。它们只演示参数格式，不包含装置数据或运行结果。
