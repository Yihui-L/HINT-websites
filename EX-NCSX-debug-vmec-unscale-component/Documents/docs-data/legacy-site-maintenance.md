...
> 本文保留原建站说明和当时的操作记录，旧目录及发布流程仅作历史文本，不作为当前操作指引；链接已指向新站点。当前构建与导出请使用[迁移后维护说明](../README.md)。

在线入口：[NCSX 初始化与第 100 步结果浏览器](https://yihui-l.github.io/HINT-websites/EX-NCSX-debug-vmec-unscale-component/)。网站公开访问；原 GitHub 仓库当时为私有，原 GitHub 档案和源码链接需要仓库权限。
...
[程序手册](../../../Source-Code/)解释模型、数值方法、参数、输入输出与统计；本网站解释本次结果。源码快照与算例档案快照分别记录，避免混用。
